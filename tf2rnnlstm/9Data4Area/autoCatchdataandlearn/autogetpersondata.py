...
# 獲取左邊臉部數據
def serialleft(areaid,savetest,Savepath,dataenoughnum):
...
# 獲取右邊臉部數據
def serialrifht(areaid,savetest,Savepath,dataenoughnum):
...
if __name__ == '__main__':
    ...
    savetest = False
    dataenoughnum = 6000
    savePathlist = ["./Soarleftbi/","./Soarrightbi/"]
    ...
    for i in range(len(savePathlist)):
        if i == 0 :
            for areaid in arealist:
                if areaid == 0:
                    while input("請將美容儀器開機放置在額頭區域（左），完成後請按Enter鍵 :") !='':
                          print("重新輸入")
                    finishedid  = serialleft(areaid, savetest, savePathlist[0], dataenoughnum)
            ...
            for areaid in arealist:
                # serialrifht(areaid, savetest, savePathlist[1],dataenoughnum)
                print(savePathlist[i]+" "+str(areaid))
